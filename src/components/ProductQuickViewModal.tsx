  ...
  const { mutate: AddToCartMutation } = useMutation({
    ...
    onSuccess: (data) => {
      showToast(
        ...
        data.status ? "Added to Cart!" : "Failed to add to cart",
      );
      onClose();
      addToCart({
        id: itemData.id,
      ...
      <div className="relative bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden animate-zoom flex flex-col md:flex-row max-h-[90vh] md:max-h-[600px]">
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col overflow-y-auto">
          ...
          <div className="mb-8 flex-grow">
            <h3 className="text-sm font-bold text-redpay-dark uppercase tracking-wider mb-2">
              Description
            </h3>
            <p className="text-redpay-grey font-century leading-relaxed">
              {itemData.description ||
                "Experience the best quality with RedPay Store."}
            </p>
          </div>
          ...
          <div className="mt-auto space-y-4">
            <div className="flex gap-4">
              <div className="flex items-center border border-gray-300 rounded-full h-12 px-4 gap-4">
                ...
                <span className="font-bold text-redpay-dark w-4 text-center">
                  {quantity}
                </span>
                <button
            ...
            <div className="text-center">
              <span className="text-xs text-redpay-grey">
                ...
                <span className="font-bold text-redpay-dark">RedPay</span>
              </span>
            </div>
          </div>
        </div>
